...
Window.prototype.create = function (url) {
    ...
    // 默认开启node环境(现在各端不太好变动，在这里统一一下)
    if (!this.setup.webPreferences) {
        this.setup.webPreferences = {
            nodeIntegration: true
        }
    } else {
        this.setup.webPreferences.nodeIntegration = this.setup.webPreferences.nodeIntegration !== false
    }

    // 根据环境变量判断是否加载预加载js脚本
    if (global.environment && global.environment.autoPreload) {
        this.setup.webPreferences.preload = global.environment.preloadPath
    }
    ...
    // On load failure
    this.object.webContents.on('did-fail-load', function (...args) {
        if (instance) instance.down(...args);
    });
 ...
 * */
Window.prototype.down = function (...args) {
    ...
    'bridge': {
        ...
        'emit': function (event, data, target) {
            windowManager.eventEmitter.emit(event, {
                'emittedBy': windowManager.getCurrent() ? windowManager.getCurrent().name : '',
                'target': target,
                'data': data
            });
        }
